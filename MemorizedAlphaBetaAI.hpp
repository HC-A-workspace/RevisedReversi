...
private:
	...
	static void alpha_beta(Node& node, const double depth_, int alpha, int beta) {
		if (depth_ <= 0) {
...
public:
	...
	MemorizedAlphaBetaAI(const double depth_ = 3) : AI(), depth(depth_) {};
	...
	Cell choose_move() override {
		...
		//auto worker = [&](std::shared_ptr<Node> child) {
		//	alpha_beta(*child, depth, INT_MIN + 1, INT_MAX - 1);
		//	};


		for (auto& child : root->get_children()) {
			threads.emplace_back(std::thread(&MemorizedAlphaBetaAI::alpha_beta, std::ref(*child), depth, INT_MIN + 1, INT_MAX - 1));
			//alpha_beta(*child, depth, INT_MIN + 1, INT_MAX - 1);
		}
